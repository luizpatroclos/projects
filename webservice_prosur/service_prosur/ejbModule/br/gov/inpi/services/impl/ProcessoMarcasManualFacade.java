...
@Stateless(name = "ProcessoMarcasManualFacade")
public class ProcessoMarcasManualFacade implements ProcessoMarcasManualInterface{

	@PersistenceContext(unitName = "prosur_marcas")
	protected EntityManager entityManager;
	
	private static final Logger LOGGER = Logger.getLogger(ProcessoMarcasManualFacade.class);
	
	protected EntityManagerFactory entityManagerFactory;
	
	public ProcessoMarcasManualFacade(){}
	...
	public boolean verificarRpiExiste(String numeroRpi){
		...
		try {
			...
			strBuilder.append("SELECT no_rpi FROM mrc_despacho where no_rpi =" + numeroRpi);
	...
	public List<Object[]> listarPedidosMarcasRetroativo(String quantidadeMarcas) {
		...
		try {
			...
			StringBuilder strBuilder = new StringBuilder();
			strBuilder.append(" SELECT FIRST " + quantidadeMarcas + " mp.cd_process,  mp.no_process, mp.dt_deposit_process, ");
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	@SuppressWarnings("unchecked")
	public List<Object[]> listarPorProcessoMarcas(String procs) {
		...
		try {
			...
			StringBuilder strBuilder = new StringBuilder();
			strBuilder.append(" SELECT mp.cd_process,  mp.no_process, mp.dt_deposit_process, ");
			strBuilder.append(" mp.dt_Publicacao, cp.nm_Complet_Pfpj, mp.nm_Marca_Process, ");
			strBuilder.append(" ma.ds_Apresen_Marca,mp.dt_Concess_Marca, mp.dt_Vigencia_Marca, ");
			strBuilder.append(" mp.dt_atualizacao_ipas, msp.cd_situaca_process,msp.st_Prosur ");
			strBuilder.append(" FROM  Mrc_Processo mp, Mrc_Situaca_Proces msp,  ");
			...
			LOGGER.error(" Sem Pedidos Por Processos ");
		}
	...
	public List<Object[]> listarPedidosMarcasSemanal(String numRpi) {
		...
		try {
			...
			strBuilder.append(" WHERE md.no_rpi = " + numRpi);
	...
	public Boolean validaProceso(String processo) {
		...
		try{
			...
			StringBuilder strBuilder = new StringBuilder();
			strBuilder.append(" SELECT no_process ");
			strBuilder.append(" FROM mrc_processo ss");
			strBuilder.append(" WHERE ss.no_process = '"+processo+"'");
			Query query = this.entityManager.createNativeQuery(strBuilder.toString());
			numProc = (String) query.getSingleResult();
